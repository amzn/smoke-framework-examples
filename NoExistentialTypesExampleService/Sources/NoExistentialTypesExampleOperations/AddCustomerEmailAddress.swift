...
extension NoExistentialTypesExampleModel.AddCustomerEmailAddressRequest: NoExistentialTypesExampleModel.CustomerEmailAddressAttributesShape { }

typealias CustomerEmailAddressRowIdentity = (emailAddress: String, rowKey: StandardCompositePrimaryKey)

private actor CustomerEmailAddressRowUpdater<DynamoDBTableType: DynamoDBCompositePrimaryKeyTable> {
    let input: NoExistentialTypesExampleModel.AddCustomerEmailAddressRequest
    let customerKey: StandardCompositePrimaryKey
    let dynamodbTable: DynamoDBTableType
    private(set) var customerEmailAddressRowIdentity: CustomerEmailAddressRowIdentity?
    
    init(input: NoExistentialTypesExampleModel.AddCustomerEmailAddressRequest,
    ...
    func updatedPayloadProvider(payload: CustomerIdentityRow) async throws -> CustomerIdentityRow {
        ...
        guard payload.customerEmailAddressSummary.emailAddresses.count < maximum else {
            ...
            throw NoExistentialTypesExampleModel.NoExistentialTypesExampleError.customerEmailAddressLimitExceeded(
    ...
    -> CustomerEmailAddressRowIdentity {
        let customerEmailAddressID = (OperationsAttributes.customerEmailAddressPrefix + [currentEmailAddress]).dynamodbKey
        ...
        let row = CustomerEmailAddressRow(attributes: input.asNoExistentialTypesExampleModelCustomerEmailAddressAttributes(),
        ...
        do {
            ...
            throw NoExistentialTypesExampleError.customerEmailAddressAlreadyExists(customerEmailAddressAlreadyExistsFault)
        }
...
extension NoExistentialTypesExampleOperationsContext {
    public func handleAddCustomerEmailAddress(input: NoExistentialTypesExampleModel.AddCustomerEmailAddressRequest) async throws
    -> NoExistentialTypesExampleModel.CustomerEmailAddressIdentity {
        guard let customerID = OperationsAttributes.externalCustomerPrefix.dropAsDynamoDBKeyPrefix(from: input.id) else {
        ...
        let customerEmailAddressRowUpdater = CustomerEmailAddressRowUpdater(input: input,
                                                                            ...
                                                                            dynamodbTable: self.dynamodbTable)
        ...
        do {
            ...
            throw NoExistentialTypesExampleError.concurrency(concurrencyFault)
        } catch SmokeDynamoDBError.conditionalCheckFailed {
            // if the row exists, make sure it is deleted
            if let rowIdentity = await customerEmailAddressRowUpdater.customerEmailAddressRowIdentity {
                try await self.dynamodbTable.deleteItem(forKey: rowIdentity.rowKey)
            ...
            throw NoExistentialTypesExampleError.unknownResource(unknownResourceFault)
        }
        ...
        return NoExistentialTypesExampleModel.CustomerEmailAddressIdentity(id: rowIdentity.emailAddress)
    }
}

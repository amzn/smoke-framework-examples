...
extension NoExistentialTypesExampleOperationsContext {
    public func handleGetCustomerDetails(input: NoExistentialTypesExampleModel.GetCustomerDetailsRequest) async throws
    -> NoExistentialTypesExampleModel.CustomerAttributes {
        guard let customerIDKeyPostfix =
            OperationsAttributes.externalCustomerPrefix.dropAsDynamoDBKeyPrefix(from: input.id) else {
                throw SmokeOperationsError.validationError(reason: "Invalid customer ID '\(input.id)")
        }
        
        let customerKey = (OperationsAttributes.customerKeyPrefix + [customerIDKeyPostfix]).dynamodbKey
